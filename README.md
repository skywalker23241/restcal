...
- 加班改为独立附加记录，可与当天出勤/请假并存，支持自定义工时、倍率、批量范围和调休额度换算。
- 新增可筛选的 RFC 5545 日历导出；CSV 完整保存加班字段，并兼容旧版备份。
...
- 五种状态一键标记：出勤、事假、病假、年假、调休；加班作为独立记录与当天状态并存，周末也可以记录加班或备注。
...
| **桌面版** | Windows 免浏览器使用 | 从 [v1.6.0 Release](https://github.com/skywalker23241/restcal-abohack/releases/tag/v1.6.0) 下载便携版、安装版或 ZIP |
...
| 下载 | 用途 |
|---|---|
| [`休历-1.6.0-portable.exe`](https://github.com/skywalker23241/restcal-abohack/releases/download/v1.6.0/%E4%BC%91%E5%8E%86-1.6.0-portable.exe) | 免安装，下载后直接运行 |
| [`休历-1.6.0-setup.exe`](https://github.com/skywalker23241/restcal-abohack/releases/download/v1.6.0/%E4%BC%91%E5%8E%86-1.6.0-setup.exe) | Windows 安装程序，可选择安装目录 |
| [`休历-1.6.0-win.zip`](https://github.com/skywalker23241/restcal-abohack/releases/download/v1.6.0/%E4%BC%91%E5%8E%86-1.6.0-win.zip) | 解压后运行 `休历.exe` |
...
`用户设置(JSON)` 只在首条数据行写入一次，其中包含个人资料、工作制度、工资、额度、请假条默认值、主题和语言。即使当前没有任何日历记录，导出的 CSV 也会保留一行设置数据。旧版不含加班字段或用户设置列的 CSV 仍可正常导入。
